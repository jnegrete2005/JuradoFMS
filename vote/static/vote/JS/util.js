...
export function addInputs(lenght, data, first = false) {
    ...
    Array.from(document.getElementsByClassName('comp-container')).forEach((comp_container, i) => {
        for (let j = 0; j < lenght; j++) {
            ...
            if (data) {
                if (mode.startsWith('min')) {
                    input.nextElementSibling.checked = data.data[`comp${i + 1}`].mode[j + 9] === 1 ? true : false;
                }
                if (comp_container.id === 'comp-1-container') {
